...
- **📱 移动端适配**: 支持手机 Safari、Chrome 访问，侧边栏自动变为抽屉菜单，各页面响应式布局
- **🔒 安全认证**: JWT Token 认证保护
...
- **🆕 磁力链接 → .torrent 自动获取**：通过 `itorrents.org` 在线缓存获取种子文件，本地缓存到 `data/torrents/`
- **🆕 广告文件智能过滤**：
  - 禁用非视频扩展名（.url / .html / .txt / .apk / .chm / .zip / .exe）
  - 禁用小视频文件（< 100MB 的广告视频）
  - 保留主视频和所有 VR 多分卷视频（PART1/2/3、-A/-B/-C）
- 提交到 BitComet 下载（支持 `torrent_file` 参数创建任务）
...
**最新功能**: v0.4.1 修复侧边栏图标过大且无法点击的问题（2026-04-22）- 将 `SidebarContent` 从 `h()` 渲染改回 `<template>` 语法

*最后更新: 2026-04-22* (v0.4.0 新增 Web UI 移动端适配 + v0.4.1 bug 修复)
